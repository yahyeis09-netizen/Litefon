...
export default function CountryRates({ onRatesClick }: CountryRatesProps) {
  ...
  const CountryCard = ({ country }: { country: typeof countries[0], key?: string }) => (
    <motion.div
      className="relative flex-shrink-0 w-64 h-80 rounded-3xl overflow-hidden shadow-xl group"
    >
      {/* Background Flag */}
  ...
  return (
    <section ref={containerRef} className="py-24 bg-white dark:bg-bg-dark transition-colors duration-300 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="text-center mb-24">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-gray-200 dark:border-white/10 bg-white dark:bg-zinc-800 mb-8"
          >
            <Globe className="w-3 h-3 text-primary-blue" />
            <span className="text-[10px] font-bold uppercase tracking-widest text-gray-500 dark:text-zinc-400">Popular Country</span>
          </motion.div>
          ...
          <motion.p
            ...
            className="text-gray-400 dark:text-zinc-400 text-lg md:text-xl max-w-2xl mx-auto font-light"
          >
        ...
        <div className="relative">
          {/* Panoramic Fades */}
          <div className="absolute inset-y-0 left-0 w-32 md:w-64 bg-gradient-to-r from-white dark:from-bg-dark via-white/80 dark:via-bg-dark/80 to-transparent z-10 pointer-events-none" />
          <div className="absolute inset-y-0 right-0 w-32 md:w-64 bg-gradient-to-l from-white dark:from-bg-dark via-white/80 dark:via-bg-dark/80 to-transparent z-10 pointer-events-none" />
